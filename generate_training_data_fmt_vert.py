...
OUTPUT_PREFIX = 'jpn_vert.fmt.exp0'

CHARS_PER_LINE = 50
CELL_SIZE = 32
ROW_H = CELL_SIZE + 2*CELL_PAD  # vert

ANK16_OFFSET = 0x3D800  # 8x16 ANK font (16 bytes per char, 256 chars)


def font_rom_code(jis1, jis2):
    """JISコード → fontRomインデックス (TOWNSEMU physmem.h FontROMCode() の移植)"""
    if jis1 < 0x28:
        # 32x8 Blocks (非漢字)
        BLK = (jis2 - 0x20) >> 5
        x   = jis2 & 0x1F
...
def get_kanji_data(rom, jis1, jis2):
    """JIS → 32バイト (16×16px)。row r: data[r*2]=左, data[r*2+1]=右"""
    code   = font_rom_code(jis1, jis2) & 0x1FFF
    offset = 32 * code
    if offset < 0 or offset + 32 > len(rom):
        return None
...
def get_ank16_data(rom, code):
    """ANKコード → 16バイト (8×16px)。1バイト/行、MSB=左端"""
...
def main():
    ...
    for c in range(0x20, 0x7F):
        ...
        chars_to_train.append({'char': chr(c), 'type': 'ank16', 'data': data})
