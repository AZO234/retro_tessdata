...
OUTPUT_PREFIX = 'jpn.f20.exp0'

CHARS_PER_LINE = 50
CELL_H = 24
CHAR_H = 20  # actual glyph height
ROW_H  = CHAR_H if TIGHT_PACK else CELL_H
F20_BASE   = 0x2810
...
def main():
    ...
    print(f"Generating multipage TIF (F20 20x20px) ({num_lines * sum(r for _,r in STYLES)} pages, {total_chars} chars × {len(STYLES)} styles)...")
    ...
    for style_name, repeat in STYLES:
        for _ in range(repeat):
            for l_idx in range(num_lines):
                ...
                line_width = len(line_chars) * (20 + 2*CELL_PAD)
                ...
                for item in line_chars:
                    ...
                    # 20×20px: 3バイト/行、上位20ビット有効
                    ox = 0
                    oy = 0 if TIGHT_PACK else (CELL_H - 20) // 2
                    d  = item['data']
                    for r in range(20):
                        val = (d[r*3] << 16) | (d[r*3+1] << 8) | d[r*3+2]
                        for bit in range(20):
                            if val & (0x800000 >> bit):
                                draw.point((x_pos + ox + bit, oy + r), fill=255)

                    all_box_entries.append(
                        f"{item['char']} {x_pos} 0 {x_pos + 20 + 2*CELL_PAD} {ROW_H} {current_page}")
                    x_pos += 20 + 2*CELL_PAD
