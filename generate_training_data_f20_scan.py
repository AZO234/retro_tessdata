...
OUTPUT_PREFIX = 'jpn.f20_scan.exp0'

CHARS_PER_LINE = 50
CELL_H = 40
ROW_H = CELL_H + 2*CELL_PAD  # f20_scan: no height reduction
F20_BASE   = 0x2810
...
def main():
    ...
    print(f"Generating multipage TIF (F20 Scanline) ({num_lines * sum(r for _,r in STYLES)} pages, {total_chars} chars × {len(STYLES)} styles)...")
        ...
        for _ in range(repeat):
            for l_idx in range(num_lines):
                ...
                for item in line_chars:
                    ...
                    # 20×20px をスキャンライン（縦2倍、1行おき）で描画 → 20×40px
                    ox = 0
                    oy = 0
                    d  = item['data']
                    for r in range(20):
